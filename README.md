...
- **VS Code extension never reaches `done`** — the Claude Code VS Code extension does not fire the `Stop` hook ([claude-code #40029](https://github.com/anthropics/claude-code/issues/40029), [#49851](https://github.com/anthropics/claude-code/issues/49851)). Sessions started from the extension chat panel only emit `PreToolUse`/`PostToolUse`/`SessionStart`/`UserPromptSubmit`, so they go `working` → `stale` and never show `done`. Run Claude from the **integrated terminal** (`claude`) for accurate `done`/green status.
